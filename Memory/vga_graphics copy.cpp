...
#define H_ACTIVE   822    // (active + frontporch - 1) - one cycle delay for mov
#define V_ACTIVE   599    // (active - 1)
#define RGB_ACTIVE 410    // (horizontal active)/2 - 1

#define TXCOUNT 120000
...
void initVGA() {
    ...
    //rgb2_program_init(pio, rgb3_sm, rgb2_offset, 0);
    ...
    //pio_enable_sm_mask_in_sync(pio, ((1u << hsync_sm) | (1u << vsync_sm) | (1u << rgb_sm) | (1u << rgb3_sm)));
    ...
    //     &pio->txf[rgb3_sm],          // write address (RGB PIO TX FIFO)
    //     &vga_data_array,            // place holder for first load from channel one
    //     TXCOUNT,                    // Number of transfers; in this case each is 1 byte.
    //     true                       // Don't start immediately.
    // );
